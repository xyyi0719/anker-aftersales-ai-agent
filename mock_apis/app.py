...
from mock_apis.routes import orders, warranty, tickets, policy
from mock_apis.engine import chat
app = FastAPI(title='Anker 售后 Mock API', version='1.1.0')
@app.middleware('http')
...
for router in (orders.router,warranty.router,tickets.router,policy.router): app.include_router(router)
@app.get('/healthz')
@app.get('/health')
def health():
    return dict(status='ok',mock=True,embedding_required=False,version='1.1.0')
